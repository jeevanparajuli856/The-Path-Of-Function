# Ren'Py + Vertex + DB Integration Master Guide

This document is the single source of truth for integrating the Ren'Py web game, frontend bridge, backend event logging, and Vertex AI helper chatbot.
...
- Vertex retrieval is restricted to game-approved corpus.
...
## Backend and DB Integration Contract (Research + Vertex)
...
### Research + Vertex DB Schema (Recommended)
...
5. `vertex_traces` (optional but strongly recommended for research)
  - `id` (PK)
  - `session_id` (FK -> `game_sessions.id`)
  - `chat_message_id` (FK -> `chat_messages.id`)
  - `model_name`
...
1. Hint effectiveness by topic:
  - Join `chat_messages` + subsequent `game_events` (`quiz_submitted`) by `session_id` and time.

2. Spoiler policy audit:
  - Filter assistant messages where `spoiler_guard = strict` and review associated `chat_citations`.

3. Retrieval quality tracking:
  - Track `vertex_traces.retrieved_chunks_json` against user follow-up success.

## Vertex RAG Corpus Preparation

Index only game-approved sources:
- scene teaching content,
- glossary/definitions,
- quiz questions and rationales,
- in-game code examples/explanations,
- helper persona/tone guidelines.
